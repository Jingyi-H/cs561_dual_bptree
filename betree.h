...
public:
    ...
    {
        /**
         * insert to tail node from tail first
         **/
        
        // std::pair<key_type, value_type> element_to_insert[] = {std::pair<key_type, value_type>(key, value)};
        // int num_to_insert = 1;
        std::pair<key_type, value_type> element_to_insert = std::pair<key_type, value_type>(key, value);
        is_tail_sorted = false;

        // now add to tree
        // case 1: tail leaf is null -> meaning tree is empty
        
        // insert to the tail leaf, see if tail is going to split
        bool split = tail_leaf->insertInLeaf(element_to_insert);

        if (split) {
            // sort elements in the tail node
            if (!is_tail_sorted) {    
                tail_leaf->sort();
                is_tail_sorted = true;
            }
