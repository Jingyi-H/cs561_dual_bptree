...
void testDualTree(std::vector<int> data, std::vector<int> queries, int num_sd, int failThres){
   // Instantiate dual bplus tree
    cout<<"TEST DUAL B+ TREE INSERT:"<<endl;
    DualBeTree<int,int> dualbptree = DualBeTree<int,int>(num_sd, failThres);
...
int main(int argc, char **argv)
{
    if (argc < 4)
    {
        std::cout << "Usage: ./main <input_file> <numOfStdev> <failThreshold>" << std::endl;
        return 0;
    }

    std::string input_file = argv[1];
    int num_sd = atoi(argv[2]);
    int failThres = atoi(argv[3]);
    ...
    testDualTree(data, queries, num_sd, failThres);
